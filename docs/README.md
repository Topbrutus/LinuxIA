...
- scripts/linuxia-repair.sh (auto-réparation ciblée en cas d'échec healthcheck)

Les fichiers générés (STATE_VM100.md, CONFIGSNAP_LATEST.txt, etc.) sont ignorés par git.

Auto-réparation (VM100):
- service systemd: services/systemd/linuxia-repair.service (déclenché par linuxia-repair.path)
